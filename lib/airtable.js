...
// Airtable REST returns linked fields as id strings and singleSelect as strings.
// These helpers tolerate both that and the enriched {id,name} object shape.
const linkId  = x => (x && typeof x === 'object' ? x.id : x);
const selName = v => ((v && typeof v === 'object' ? v.name : v) || '');
// The fixed 4+2 drivers, by their record id in the Drivers table.
const DRIVER_BY_ID = {
...
async function getQuestionMap(){
  ...
  for(const r of recs){
    ...
    const funcs = Array.isArray(c[F.q.func]) ? c[F.q.func].map(selName).filter(Boolean) : [];
    map[r.id] = {
      id: r.id,
      text: c[F.q.text] || '',
